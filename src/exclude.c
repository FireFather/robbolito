...
#ifndef BUILD_EXCLUDE
#define BUILD_EXCLUDE
#include "robbolito.h"
#include "history.h"
#include "move_null.h"
#include "exclude.c"
...
int my_exclude( int VALUE, int DEPTH, uint32 MOVE )
    {
    int move, i;
    int k;
    type_zobrist *tr;
    ...
    (POSITION_0 + 1)->move = 0;
    repetition_check;
    zob ^= ZOBRIST[count_my_king][DI(MOVE)] ^ ZOBRIST[count_your_king][AI(MOVE)];
    k = zob & list_hide;
    ...
    while( (move = my_next(NEXT)) )
        ...
        if( cnt > 5 && NEXT->phase == move_ordinary && (move & 0xe000) == 0 && square_fixed[di] & ~my_xray
            && DEPTH < 20 )
            {
            if( (6 << (DEPTH - 6)) + MAXIMUM_POSITIONAL (move) +
            (POSITION_0->value) < VALUE + 30 + 2 * cnt )
                {
                cnt++;
                continue;
                }
        ...
        if( check_move )
            ...
            if( NEXT->phase == move_ordinary && (cnt >= 3 || REDUCTION) )
                {
                hole_new = DEPTH - 2 + EXTENSION - BSR(1 + cnt) - REDUCTION;
    ...
    v = VALUE - 1;
    Zobrist_high(zob, DEPTH, v);
    RETURN(v);
    }

int my_exclude_check( int VALUE, int DEPTH, uint32 MOVE )
    {
    ...
    while( p->move || !GEN )
        ...
        Zobrist_low(zob, move, MAXIMUM(1, DEPTH), v);
        RETURN(v);
        }
    Zobrist_high(zob, MAXIMUM(1, DEPTH), value_better);
    RETURN(value_better);
    }
