...
void information( sint64 x, int ALPHA, int value, int BETA )
    ...
    *q = 0;
    ...
    printf("info depth %d seldepth %d score %s %s time %I64u nodes %I64u nps %I64u pv %s", root_hole / 2, sd,
        cp_mate(value, STRING2), modifier(ALPHA, value, BETA, STRING3), t, NODES, nps * 1000, pv);
    if( FIRST_TIME && x > 200000)
    {
        printf(" cpuload %u", (unsigned)((double)(process_clock() - MOMENT) / (double)(x * numCPU) * 1000));
        FIRST_TIME = FALSE;
    }
    printf("\n");
    fflush(stdout);
